...
output_file = 'caviar_dataset_128x48_l_h_caviar_dataset_128x48_l_hl_norm_weight_feature_loss_new_0_iter_1000_fc7_bn'
...
# for theta in np.arange(1, 2):
# for theta in np.arange(0.1, 1.1, 0.1):
  # len(resolution) is the number of the probe images
  for i in xrange(len(resolution)):
      D0_weight = (1/math.sqrt(2*math.pi*(theta**2)))*math.exp(-1*((resolution[i]-2)**2)/(2*(theta**2)))
      D1_weight = (1/math.sqrt(2*math.pi*(theta**2)))*math.exp(-1*((resolution[i]-3)**2)/(2*(theta**2)))
      D2_weight = (1/math.sqrt(2*math.pi*(theta**2)))*math.exp(-1*((resolution[i]-4)**2)/(2*(theta**2)))
