...
D0_file = osp.join(results_dir_x2,'distance_weight_euclidean_0.1.npy')
D1_file = osp.join(results_dir_x3,'distance_weight_euclidean_0.5.npy')
D2_file = osp.join(results_dir_x4,'distance_weight_euclidean_0.3.npy')
D0 = np.load(D0_file)
D1 = np.load(D1_file)
D2 = np.load(D2_file)
...
D = (D0+D1+D2)/3
C = cmc(D, GY, PY)
np.save(osp.join(output_dir, 'cmc_' + 'average_euclidean_weight'), C)
np.save(osp.join(output_dir, 'distance_' + 'average_euclidean_weight'), D)
