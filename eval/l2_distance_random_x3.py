...
def _get_test_data(result_dir):
    PX = np.load(osp.join(result_dir, 'test_probe_features_x3.npy'))
...
def main(args):
    ...
    print(D.shape)
    np.save(osp.join(args.result_dir, 'metric_' + file_suffix + '_x3'), M)
    np.save(osp.join(args.result_dir, 'cmc_' + file_suffix + '_x3'), C)
    np.save(osp.join(args.result_dir, 'distance_' + file_suffix + '_x3'), D)
