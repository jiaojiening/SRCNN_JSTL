...
def _eval_cmc(PX, PY, GX, GY, M):
    ...
    C = cmc_single_probe(D, GY, PY)
    return D, C

def main(args):
    # X, Y = _get_train_data(args.result_dir)
    
    # Change probe and the gallery for testing
    # Means probe--normal-resolution(cam_1), gallery--low-resolution(cam_0)
    # PX, PY, GX, GY = _get_test_data(args.result_dir)
    GX, GY, PX, PY = _get_test_data(args.result_dir)
    ...
    M = np.eye(PX.shape[1])
    D, C = _eval_cmc(PX, PY, GX, GY, M)
    print(D.shape)
    # np.save(osp.join(args.result_dir, 'metric_' + file_suffix), M)
    np.save(osp.join(args.result_dir, 'cmc_exchange_single_probe_' + file_suffix), C)
    np.save(osp.join(args.result_dir, 'distance_exchange_single_probe_' + file_suffix), D)
