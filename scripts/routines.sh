...
# Some constants
CAFFE_DIR=/home/jiening/dgd_person_reid/external/caffe

EXP_DIR=/home/share/jiening/dgd_datasets/exp
DATASETS_DIR=${EXP_DIR}/datasets
DB_DIR=${EXP_DIR}/db

EXP_DIR=/home/jiening/SRCNN_JSTL/external
RESULTS_DIR=${EXP_DIR}/results
SNAPSHOTS_DIR=${EXP_DIR}/snapshots
NIS_DIR=${EXP_DIR}/nis

MODELS_DIR=/home/jiening/SRCNN_JSTL/models
LOGS_DIR=/home/jiening/SRCNN_JSTL/logs
...
get_result_dir() {
  ...
  local weights_name="${weights_name%%.caffemodel}"
...
train_model() {
  ...
  # Training
  if [[ $# -eq 2 ]]; then
    GLOG_logtostderr=1 mpirun -n 1 ${CAFFE_DIR}/build/tools/caffe train \
      -solver ${solver} -gpu 0,1 2>&1 | tee ${log}
  else
    GLOG_logtostderr=1 mpirun -n 1 ${CAFFE_DIR}/build/tools/caffe train \
      -solver ${solver} -weights ${pretrained_model} -gpu 1 2>&1 | tee ${log}
  fi
}
...
extract_features_mixture() {
  ...
  for subset in test_probe; do
    ...
    local model=$(mktemp)
    sed -e "s/\${dataset}/${dataset}/g; s/\${subset}/${subset}/g" \
      ${MODELS_DIR}/exfeat_template_data_b.prototxt > ${model}
    ${CAFFE_DIR}/build/tools/extract_features \
      ${trained_model} ${model} ${blob},label \
      ${result_dir}/${subset}_features_lmdb,${result_dir}/${subset}_labels_lmdb \
      ${num_iters} lmdb GPU 1
    python2 tools/convert_lmdb_to_numpy.py \
      ${result_dir}/${subset}_features_lmdb ${result_dir}/${subset}_features.npy \
      --truncate ${num_samples}
    python2 tools/convert_lmdb_to_numpy.py \
