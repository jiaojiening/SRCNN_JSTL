...
#basic setting
#lmdb_file = '/home/jiening/SRCNN_JSTL/external/db/jstl_cuhk01_ln_market/val_cam1_lmdb'
lmdb_file = '/home/jiening/SRCNN_JSTL/external/db/jstl_cuhk01_ln_market/train_cam1_lmdb'
...
for line in open(txt_file):  
    ...
    if dataset == 'market':
        ...
        if cam == 'cam_1':
            count+=1
            img_b = cv2.imread(join(cuhk01_data_b_path_x2, cam, file_name))
            img_b = cv2.resize(img_b, (resize_width, resize_height)) 
            ...
            count+=1
            img_b = cv2.imread(join(cuhk01_data_b_path_x3, cam, file_name))
            img_b = cv2.resize(img_b, (resize_width, resize_height)) 
            # img = cv2.imread(join(data_path,file_name))
            # img = cv2.resize(img, (resize_width, resize_height)) 
            img_new = np.append(img_b, img, axis = 2)
            ...
            count+=1
            img_b = cv2.imread(join(cuhk01_data_b_path_x4, cam, file_name))
            img_b = cv2.resize(img_b, (resize_width, resize_height)) 
            # img = cv2.imread(join(data_path,file_name))
            # img = cv2.resize(img, (resize_width, resize_height)) 
            img_new = np.append(img_b, img, axis = 2)  
